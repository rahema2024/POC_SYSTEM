...
// (غير مفعّلة حاليًا لكن تبقى للجاهزية)
...
async function ensureSheets() {
  ...
  if (!titles.has(SHEET_CUSTOMERS)) req.push({ addSheet: { properties: { title: SHEET_CUSTOMERS } } });
  if (!titles.has(SHEET_DRIVERS)) req.push({ addSheet: { properties: { title: SHEET_DRIVERS } } });
  if (req.length) {
    await sheets.spreadsheets.batchUpdate({ spreadsheetId: SPREADSHEET_ID, requestBody: { requests: req } });
  }

  // Orders headers (A..P) - أضفنا delivery_note في النهاية
...
app.get('/driver/claim', async (req, res) => {
  try {
    ...
    const driverPhone = req.query.d;

    if (!orderId || !driverPhone)
      return res.send('❌ بيانات ناقصة');
    ...
    if (rowIndex === -1) return res.send('❌ الطلب غير موجود');
    ...
    res.send('✅ تم استلام الطلب — شكراً');
  } catch (e) {
    console.error(e);
    res.send('❌ خطأ داخلي');
  }
...
// تمّ التوصيل (Delivered) + ملاحظة اختيارية
